...
const AddTicket = () => {
  ...
  const validate = () => {
    ...
    if (!form.startTime || !form.endTime) newErrors.time = "Enter start and end times";
  ...
  const formatTime = (timeStr) => {
    ...
    return `${hr12.toString().padStart(2, "0")}:${minute.toString().padStart(2, "0")} ${ampm}`;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedTime = `${formatTime(form.startTime)} - ${formatTime(form.endTime)}`;
    ...
    <div className="flex flex-col items-center justify-center p-8 min-h-screen bg-gray-50">
      ...
      <Card color="transparent" shadow={false} className="p-6 w-full max-w-lg">
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div>
            <label className="text-sm text-brand-secondary mb-1 block">Event Category</label>
            <select
          ...
          <div>
            <label className="text-sm text-brand-secondary mb-1 block">Ticket Categories</label>
            {form.ticketCategories.map((ticket, index) => (
              <div key={index} className="flex gap-2 mb-2 items-center">
                ...
                {form.ticketCategories.length > 1 && (
                  <Button onClick={() => removeTicketCategory(index)} className="bg-brand-dark">
                    remove
                  </Button>
                )}
              </div>
            ))}
            <Button onClick={addTicketCategory} className="bg-brand-secondary w-full mt-2">
               Add Ticket Category
            </Button>
          </div>
          ...
          {errors.time && <p className="text-red-500 text-sm -mt-4">{errors.time}</p>}
